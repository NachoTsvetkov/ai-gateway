...
export function DemoLauncher() {
  return (
    <div className="fixed right-5 bottom-24 z-40 flex flex-col items-end gap-2">
      {DEMOS.map((demo) => (
        <Link
          key={demo.href}
          href={demo.href}
        ...
        >
          <span className="rounded-full border border-neutral-700 bg-neutral-800 px-2.5 py-1 text-[11px] font-medium text-neutral-300 transition-colors group-hover:border-blue-500/50 group-hover:text-white">
            {demo.label}
          </span>
          <span
            className={`flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br ${demo.gradient} text-white shadow-lg transition-transform hover:scale-110 active:scale-95`}
          >
            {demo.icon}
          </span>
        </Link>
