...
Durable, non-obvious notes for running this app in the Cloud Agent VM (Linux/bash, not the Windows/pwsh workflow the rest of this file assumes). Standard commands live in `package.json` scripts and the README; only the caveats are here.

- **Package manager is npm, not pnpm.** The repo has a committed `package-lock.json` and no `pnpm-lock.yaml`, so despite README/AGENTS references to `pnpm`, dependency installs must use npm. The startup update script runs `npm ci`. Run scripts with `npm run <script>` (e.g. `npm run dev`, `npm run build`, `npm run prettier:check`).
- **Create `.env.local` before running:** `cp .env.example .env.local`. `.env.example` already contains the real, public `NEXT_PUBLIC_FIREBASE_*` and Meta Pixel values, so this alone is enough to make the Firebase-backed lead-capture flows work. `.env.local` is gitignored and is NOT recreated by the update script — recreate it if missing. Missing secrets degrade gracefully (see below); the app still boots without them.
- **Dev server:** `npm run dev` (Turbopack, port 3000). The marketing site (home, `/bundles/*`, `/services`, `/projects`, the `/free-ai-audits` + `/ai-*` audit funnels) and the KORE (`/projects/local-fitness-studio`) and ROZÉ (`/projects/boutique-fashion-brand`) brand demos all render with no external secrets.
- **Shopify storefront needs credentials.** Routes backed by the Shopify Storefront API — `/projects/ai-shopify-store`, `/search/[collection]`, `/product/[handle]`, and `/projects/rag-demo` — return 500 in dev until `SHOPIFY_STORE_DOMAIN` + `SHOPIFY_STOREFRONT_ACCESS_TOKEN` are set. The `lib/shopify` layer no-ops (returns `[]`) for menus/collections but throws for product/collection fetches, which is the 500 source.
- **`npm run build` requires Shopify creds.** A full production build fails while prerendering the Shopify pages (e.g. `/search/[collection]`) without those two env vars. This is expected; use `npm run dev` for development. Only chase build failures that are unrelated to missing Shopify config.
- **AI agents need `AI_GATEWAY_API_KEY`.** The sales assistant, visual stylist, and other `app/api/*/chat/route.ts` agents stream via the Vercel AI Gateway and won't respond without it. The UI widgets still mount.
- **Firebase test flow (the safe end-to-end check):** `npm run test:surveys` (dev server must be running) submits a report request through `/api/report-request?test=true` and reads it back. It writes to the `*_test` Firestore collections in the real `nachotsvetkov-cbaa7` project using the committed public config. Prod forms (no `?test`) write to the live collections — do not submit those with junk data.
- **Lint/"test" = `npm run prettier:check`.** On a clean checkout this currently exits 1 with pre-existing formatting drift across many files (including committed build artifacts like `build.log`). That is the repo's existing state — do NOT mass-reformat to make it pass; only run `npm run prettier` on files you actually touched.